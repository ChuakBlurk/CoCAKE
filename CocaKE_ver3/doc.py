...
class RelGenDataset(torch.utils.data.dataset.Dataset):
    ...
    def __init__(self, path, task, commonsense_path, batch_size, cake_ratio, examples=None):
        self.path_list = path.split(',')
        self.task = task
        assert all(os.path.exists(path) for path in self.path_list) or examples
    ...
    def __getitem__(self, index):
        ...
        while len(sampled_exs) < cake_sample_cnt:
          ...
          if sampled_relations not in sampled_relations:
              sampled_exs += self.concept_filter_updated(sampled_relation, self.batch_size//8)
        ...
        sampled_exs += random.choices(list(self.examples), k=random_sample_cnt)
        return [ex.vectorize() for ex in sampled_exs[:args.batch_size]] 

    ###################### give the examples of all possible relation tails ###############
    def concept_filter_updated(self, relation, example_size): 
